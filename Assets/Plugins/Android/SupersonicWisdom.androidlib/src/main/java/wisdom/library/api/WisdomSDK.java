...
public class WisdomSDK {
    ...
    public static void initiateUpdate(Activity activity, int updateType){
        if (getInstance().isInitialized()) {
            getInstance().initiateUpdate(activity, updateType);
        }
    }
    
    public static void checkForUpdate(){
        if (getInstance().isInitialized()) {
            getInstance().checkForUpdate();
        }
    }
    
    public static void addAppUpdateListener(IWisdomAppUpdateListener listener){
        if (getInstance().isInitialized()){
            getInstance().addAppUpdateListener(listener);
        }
    }
    
    public static void removeAppUpdateListener(IWisdomAppUpdateListener listener){
        if (getInstance().isInitialized()){
            getInstance().removeAppUpdateListener(listener);
        }
    }

    public static boolean openApplication(String appIdentifier) {
        return getInstance().openApplication(appIdentifier);
    }
    ...
    public static long captureSessionBackgroundTime() { return getInstance().captureSessionBackgroundTime(); }
    
    public static long releaseSessionBackgroundTime() { return getInstance().releaseSessionBackgroundTime(); }
}
