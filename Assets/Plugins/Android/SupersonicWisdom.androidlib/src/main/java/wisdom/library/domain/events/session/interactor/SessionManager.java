...
public class SessionManager implements ISessionManager, ISessionListener {
    ...
    private final AtomicBoolean isBackgroundTimeCaptured;
    private final AtomicBoolean shouldCaptureBackgroundTime;
    ...
    public SessionManager(IEventsReporter reporter, IEventMetadataManager eventMetadataManager,
        ...
        isBackgroundTimeCaptured = new AtomicBoolean(false);
        shouldCaptureBackgroundTime = new AtomicBoolean(false);
    }
    ...
    private long privateCaptureSessionBackgroundTime() {
        synchronized (this) {
            if (shouldCaptureBackgroundTime.get()) {
                SdkLogger.log(this, "Capture already in progress, ignoring...");
                return 0;
            }
            if (!isSessionStarted()) {
                SdkLogger.log(this, "Cannot capture background time without an active session.");
                return 0;
            }

            SdkLogger.log(this, "Capture background time");
            shouldCaptureBackgroundTime.set(true);
            return 1; 
        }
    }

    private long privateReleaseSessionBackgroundTime() {
        synchronized (this) {
            if (!shouldCaptureBackgroundTime.get()) {
                SdkLogger.log(this, "Release called without a capture, ignoring...");
                return 0;
            }

            SdkLogger.log(this, "Release background time");

            shouldCaptureBackgroundTime.set(false);
            isBackgroundTimeCaptured.set(false);

            SdkLogger.log(this, "Total background time captured: " + totalBackgroundTime);
    ...
    public long captureSessionBackgroundTime() {
        return privateCaptureSessionBackgroundTime();
    }

    @Override
    public long releaseSessionBackgroundTime() {
        return privateReleaseSessionBackgroundTime();
    }
    ...
    public void onAppMovedToForeground() {
        ...
        synchronized (this) {
            if (shouldCaptureBackgroundTime.get() && isBackgroundTimeCaptured.get()) {
                long currentTime = currentTimeSeconds();
                totalBackgroundTime += (currentTime - backgroundStartTime);
                SdkLogger.log(this, "Background time paused at: " + currentTime + " with total: " + totalBackgroundTime);
    ...
    public void onAppMovedToBackground() {
        ...
        synchronized (this) {
            if (shouldCaptureBackgroundTime.get()) {
                backgroundStartTime = currentTimeSeconds();
                isBackgroundTimeCaptured.set(true);
                SdkLogger.log(this, "Background time resumed at: " + backgroundStartTime);
            }
        }
    }
