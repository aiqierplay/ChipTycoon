...
namespace Aya.Pool
{
	public class EntityPoolList
	{
		...
		/// 生成对象列表
		/// </summary>
		private readonly HashSet<GameObject> _spawnList = new HashSet<GameObject>();

		/// <summary>
		/// 销毁对象列表
		/// </summary>
		private readonly HashSet<GameObject> _deSpawnList = new HashSet<GameObject>();
		...
		/// 值
		/// </summary>
		public HashSet<GameObject> Values => _spawnList;
        ...
        public GameObject Spawn(GameObject prefab, Transform parent, Vector3 position, Action<GameObject> onSpawn = null)
        {
            ...
            {
                // 从对象池取出
                using var enumerator = _deSpawnList.GetEnumerator();
                enumerator.MoveNext();
                ins = enumerator.Current;
        ...
        public bool Contains(GameObject ins)
	    {
            foreach (var item in _spawnList)
            {
                if (item == ins) return true;
            }

            foreach (var item in _deSpawnList)
            {
                if (item == ins) return true;
            }
        ...
        public void AutoDestroy()
        {
            ...
            if (_deSpawnList.Count > 0)
            {
                ins = _deSpawnList.ElementAt(0);
                _deSpawnList.Remove(ins);
            }
            else if (_spawnList.Count > 0)
            {
                ins = _spawnList.ElementAt(0);
                _spawnList.Remove(ins);
            }
