...
namespace Aya.Util
{
	...
	public static class RandUtil
	{
		...
		public static int RandInt(Vector2Int range)
		{
			return RandInt(range.x, range.y);
		}
		...
		/// </summary>
		/// <param name="range">范围</param>
		public static float RandFloat(Vector2 range)
		{
			return RandFloat(range.x, range.y);
		}
        ...
        /// <param name="min">最小值</param>
        /// <param name="max">最大值</param>
        /// <param name="onPlane">是否处于平面上</param>
        /// <returns>结果</returns>
        public static Vector3 RandVector3(float min = float.MinValue, float max = float.MaxValue, bool onPlane = false)
        {
            return new Vector3(RandFloat(min, max), onPlane ? 0f : RandFloat(min, max), RandFloat(min, max));
        }
        ...
        /// <param name="onPlane">是否处于平面上</param>
        /// <returns>结果</returns>
        public static Vector3 RandVector3(Vector3 from, Vector3 to, bool onPlane = false)
        {
            return new Vector3(RandFloat(from.x, to.x), onPlane ? 0f : RandFloat(from.y, to.y), RandFloat(from.z, to.z));
        }
        ...
        /// <param name="onPlane">是否处于平面上</param>
        /// <returns>结果</returns>
        public static Vector3 RandVector3(Vector3 center, float radius, bool onPlane = false)
        {
            ...
            do
            {
                result = RandVector3(center - Vector3.one * radius, center + Vector3.one * radius, onPlane);
		...
		public static T Random<T>(IList<T> list, List<int> weights)
		{
			...
			var rand = RandInt(0, weightCount + 1);
			weightCount = 0;
			var index = -1;
			do
			{
		...
		public static T Random<T>(IList<T> list, List<float> weights)
		{
			...
			var rand = RandFloat(0f, weightCount + 1e-6f);
			weightCount = 0f;
			var index = -1;
			do
			{
