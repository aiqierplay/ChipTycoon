...
namespace Aya.Maths
{
    public static partial class MathUtil
    {
        ...
        public static Vector2 SLerp(Vector2 from, Vector2 to, float deltaTime)
        {
            var a = NormalizeAngleRadian(Mathf.Lerp(Mathf.Atan2(from.y, from.x), Mathf.Atan2(to.y, to.x), deltaTime));
            var l = Mathf.Lerp(from.magnitude, to.magnitude, deltaTime);
        ...
        /// <param name="from">开始</param>
        /// <param name="to">结束</param>
        /// <param name="angularSpeed">角速度(度数)</param>
        /// <param name="deltaTime">时间</param>
        /// <returns>结果</returns>
        public static Quaternion SLerp(Quaternion from, Quaternion to, float angularSpeed, float deltaTime)
        {
            var da = angularSpeed * deltaTime;
